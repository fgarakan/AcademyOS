...
| `ExecutiveWorkspaceGuardian` (the §12 certification) | **Built** | Guardian #1 of the AcademyOS Guardian Framework (`src/lib/guardians/`). Read-only; observes · classifies · certifies · reports · blocks regressions. Runs GREEN on a ratchet-only baseline of **33** accepted backlog violations (down from 38 — Sidebar Purification V1 moved the level-movement, curriculum-adjustment, and draft-decision controls to their owning review page); a new sidebar mutation/editor/multi-step/completion control fails CI (exit 1). The §12 100% ship-gate is the convergence target as the baseline shrinks to zero. |
...
2. ~~**High-risk apply controls in the sidebar.**~~ **RESOLVED (Sidebar Purification V1).** `DonnaLevelMovementApplyControls`, `DonnaCurriculumAdjustmentApplyControls`, and `DonnaIntelligenceDraftDecisionControls` were moved out of the sidebar render tree into the owning review page (`src/app/director/review/`); the sidebar review-queue panel now previews the draft and hands off to `/director/review`, where the identical controls run. `applyApprovedLevelMovementAction` / `applyApprovedCurriculumAdjustmentAction` / `updateDonnaIntelligenceDraftDecisionAction` are no longer reachable from the widget — §7.4 satisfied for these. Zero capability regression (same controls, page-owned).
...
3. **Move apply controls to their owning pages. ✅ DONE (Sidebar Purification V1, baseline 38 → 33).** `DonnaLevelMovementApplyControls` / `DonnaCurriculumAdjustmentApplyControls` / `DonnaIntelligenceDraftDecisionControls` relocated to `src/app/director/review/` and removed from the widget; the review-queue panel's terminal action for proposed-action drafts is now navigation to `/director/review`. §7.4 closed for these (highest risk first). Voice-note routing in the panel is intentionally retained — it has no page owner yet; its migration is deferred until one exists, to preserve zero capability regression.
