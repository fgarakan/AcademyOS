...
VALUES
  ...
  ('A4', 'Recreation-Oriented', 'Orange 2', 'Tennis is one of three or four activities. 1–2 sessions/wk, plays for fun, no tournament ambition. Parent values social and physical-development outcomes more than ranking. Will likely cap at Green 3 or Yellow 1 by choice.', 'AOS should support a ''Recreation flag'' on the player record that reframes Competition gates as optional rather than blocking, while keeping Skill Track gates intact. Otherwise the platform misreports their progress.'),
  ('A5', 'High-Pressure Family Context', 'Green 1', 'Talented player, parent driving the developmental timeline aggressively. Demands promotion conversations every 6–8 weeks. Player is technically capable but shows mentality flags — tightens in matches, post-loss avoidance, fragile around evidence-of-error feedback.', 'Gate objectivity. Mentality column evidence (composure, error-recovery, between-point routine adherence). AOS should log every promotion request with the evidence cited so Director has audit trail.'),
...
-- Engineering requirements. [PROPOSED:] markers stripped; content kept.
-- Product/tool names stripped from risk_description + required_response.
...
VALUES (
  ...
  'Coach',
  'Weekly',
  'If a rotational training tool warm-up is integrated, log dominant body-organization observation here.',
  2
) ON CONFLICT (gate_id) DO NOTHING;
...
VALUES (
  ...
  'Director',
  'End of 8-week block',
  'If a video assessment protocol is available at this stage as a basic recording tool, use it for the video; otherwise coach phone video.',
  11
) ON CONFLICT (gate_id) DO NOTHING;
