...
const ONBOARDING_PATTERNS = /\b(onboard|onboarding|set\s?up|setup|getting started|getting going|how to start|start using|configure academy|configure my academy|walk me through setup|help with setup)\b/i

const PLAYERS_QUESTION_PATTERNS = /\b(which players?|who needs? attention|player attention|player data|no players?|missing players?|add players?|player profile|player progress|player list|players? need|player concern)\b/i

const COACHES_QUESTION_PATTERNS = /\b(who should coach|which coach|assign coach|coach for|coach orange|coach green|coach red|coach white|coach blue|coach yellow|coaches? setup|no coaches?|add coaches?|coach availability|coaching assignment|head coach)\b/i
