...
const DIRECTOR_SIGNAL_MAP: DirectorSignalEntry[] = [
  {
    ...
    signals: [
      ...
      // Sprint 700 — P0 fix: catch natural raw-coach-note-to-parent phrases
  ...
  },
  {
    ...
    signals: [
      ...
      // Sprint 700 — P0 fix: natural "Move [name] up/down" and question forms
      /move\s+[\w'-]+\s+(up|down)/i,
      /can\s+[\w'-]+\s+move\s+(up|down)/i,
      /should\s+[\w'-]+\s+move\s+(up|down)/i,
    ],
    safetyClass: 'needs_review',
