...
const LOOPS: LoopSpec[] = [
  ...
  {
    id: 3, name: 'Class Template Setup',
  ...
  {
    id: 5, name: 'Coach Assignment & Session Readiness',
    ...
    note: 'No dedicated reassignment screen — assignment via onboarding coaches-permissions + session creation. "Session readiness" is a derived state, not its own route (documented).',
  },
  {
    id: 6, name: 'Coach Session Execution',
    routeFiles: ['src/app/coach/sessions/[sessionId]/page.tsx'],
    primaryActionFiles: ['src/app/coach/sessions/[sessionId]/CoachSessionExecutionClient.tsx'],
    primaryActionMarker: /block|status|active|complete|skip|btn-lime|onClick/i,
    donna: 'coach',
    actionFiles: ['src/app/coach/sessions/[sessionId]/updateBlockStatusAction.ts'],
    requiresApproval: false,
    approvalMarker: /assertNotPreviewMode/,
    realWriteMarker: /session_blocks|actual_status|\.update\(|writeAuditLog/i,
    note: 'Live on-court execution — block actual_status persists to session_blocks with audit log.',
  },
  {
    id: 7, name: 'Coach Wrap-Up',
  ...
  {
    id: 8, name: 'Player Development & Evidence',
    routeFiles: ['src/app/director/players/[playerId]/page.tsx', 'src/app/director/placement/page.tsx'],
    primaryActionFiles: ['src/app/director/players/[playerId]/page.tsx', 'src/app/director/placement/page.tsx'],
    primaryActionMarker: /assess|Assessment|Evaluate|placement|activate|evidence|btn-lime/i,
    donna: 'director',
    actionFiles: [
      'src/app/director/players/[playerId]/quickAssessmentAction.ts',
      'src/app/director/players/[playerId]/assessmentStudioAction.ts',
      'src/app/director/players/[playerId]/recordGateEvidenceAction.ts',
      'src/app/director/placement/placementDraftAction.ts',
    ],
    requiresApproval: true,
    approvalMarker: /finalize_player_placement|proposed_actions|pending_review|assertNotPreviewMode|audit/i,
    realWriteMarker: /finalize_player_placement|insert|update|assessment|evidence|placement|\.from\(/i,
    note: 'Player development lifecycle: placement/activation → assessment → gate evidence on one record. finalize_player_placement() is the only activation path.',
  },
  {
    id: 9, name: 'Director Review & Approval',
  ...
  {
    id: 10, name: 'Parent & Player-Safe Clarity',
    routeFiles: ['src/app/parent/page.tsx', 'src/app/player/page.tsx', 'src/app/director/players/[playerId]/page.tsx'],
    ...
    realWriteMarker: /insert|proposed_actions|\.from\(/i,
    note: 'Parent update is draft→review (never auto-sent); parent/player portals render only parent/player-safe, approved content.',
  },
]
