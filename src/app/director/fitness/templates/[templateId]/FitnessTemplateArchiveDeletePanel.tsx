...
import { archiveFitnessTemplateAction, deleteFitnessTemplateAction } from './archiveDeleteFitnessTemplateAction'
...
export function FitnessTemplateArchiveDeletePanel({ templateId, templateName, sessionCount }: Props) {
  ...
  function handleArchive() {
    ...
    startTransition(async () => {
      const result = await archiveFitnessTemplateAction(templateId)
      if (result.ok) {
        router.push('/director/fitness/templates')
      } else {
  ...
  function handleDelete() {
    setError(null)
    startTransition(async () => {
      const result = await deleteFitnessTemplateAction(templateId)
      if (result.ok) {
        router.push('/director/fitness/templates')
      } else {
        setError(result.error)
            ...
            <div className="space-y-1">
              ...
              {sessionCount > 0 ? (
                <p className="text-[11px] text-text-secondary">
                  ...
                  Archiving preserves all history. The template will no longer appear as active.
                </p>
              ) : (
                <p className="text-[11px] text-text-secondary">
                  Archiving marks this template inactive while preserving its definition.
                </p>
              )}
            </div>
          </div>
