...
import { updatePriorityRecommendationDecisionAction } from './actions'
...
export function PriorityDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updatePriorityRecommendationDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      ...
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approval marks this recommendation as ready for a future priority-creation step.
          It does not create an active priority yet.
        </span>
      </div>

      {/* Decision note */}
      <div className="space-y-1">
        <label className="label-xs" htmlFor={`prd-note-${proposedActionId}`}>
          Decision note (optional)
        </label>
        <textarea
          id={`prd-note-${proposedActionId}`}
          value={noteText}
