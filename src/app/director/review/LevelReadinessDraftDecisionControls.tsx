...
import { updateLevelReadinessReviewDecisionAction } from './actions'
...
export function LevelReadinessDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateLevelReadinessReviewDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving this readiness review does not move the player. It creates a separate level movement plan
          draft in the next review step.
        </span>
      </div>
