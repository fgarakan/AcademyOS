...
import { updateAttendanceExceptionDraftDecisionAction } from './actions'
...
export function AttendanceExceptionDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateAttendanceExceptionDraftDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving marks this draft as ready to apply. Rostered attendance will not be recorded until you explicitly apply it. Unrostered attendees are never auto-applied.
        </span>
      </div>

      <div className="space-y-1">
        <label className="label-xs" htmlFor={`note-ae-${proposedActionId}`}>
          Decision note (optional)
        </label>
        <textarea
          id={`note-ae-${proposedActionId}`}
          value={noteText}
