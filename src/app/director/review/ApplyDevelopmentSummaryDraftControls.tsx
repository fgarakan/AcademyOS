...
import { applyApprovedSummaryDraftAction } from './actions'

interface Props {
  proposedActionId: string
}

export function ApplyDevelopmentSummaryDraftControls({ proposedActionId }: Props) {
  ...
  function handleApply() {
    startTransition(async () => {
      const res = await applyApprovedSummaryDraftAction(proposedActionId)
      setResult(res)
  ...
  if (result?.ok) {
    return (
      <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl bg-status-green/10 border border-status-green/30 text-xs text-status-green">
        ...
        <span>Applied. Player development summary updated. Refreshing…</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Apply updates the player's internal development summary with the proposed strengths, work-on areas, and coach summary. This is internal only — not published to player or parent. No player level, curriculum, or parent communication is changed.
        </span>
      </div>
      <button
        onClick={handleApply}
      ...
      >
        ...
        {isPending ? 'Applying…' : 'Apply — Update Development Summary'}
      </button>
      {result?.error && (
      ...
      {isPending && <p className="text-[11px] text-text-muted">Writing development summary…</p>}
    </div>
  )
}
