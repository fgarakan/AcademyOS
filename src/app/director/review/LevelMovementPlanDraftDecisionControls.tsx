...
import { updateLevelMovementPlanDecisionAction } from './actions'
...
export function LevelMovementPlanDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateLevelMovementPlanDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving this plan does not move the player. A separate application step (Sprint 47) will
          update the player&apos;s curriculum level only after director approval.
        </span>
      </div>

      <div className="space-y-1">
        <label className="label-xs" htmlFor={`note-${proposedActionId}`}>
          Decision note (optional)
        </label>
        <textarea
          id={`note-${proposedActionId}`}
          value={noteText}
