...
import { updateCurriculumOverrideDraftDecisionAction } from './actions'
...
export function CurriculumOverrideDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateCurriculumOverrideDraftDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving does not change the curriculum yet. It marks this draft as ready for an
          Apply step that creates an official override record.
        </span>
      </div>

      <div className="space-y-1">
        <label className="label-xs" htmlFor={`note-curriculum-${proposedActionId}`}>
          Decision note (optional)
        </label>
        <textarea
          id={`note-curriculum-${proposedActionId}`}
          value={noteText}
