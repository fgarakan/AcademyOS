...
export function WrapUpObservationDraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateObservationDraftDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving marks this observation as reviewed — use the Apply button to write it as an internal coach observation. Rejecting records your decision only. No player profile, parent message, or curriculum record is touched by this action.
        </span>
      </div>

      <div className="space-y-1">
        <label className="label-xs" htmlFor={`obs-note-${proposedActionId}`}>
          Decision note (optional)
        </label>
        <textarea
          id={`obs-note-${proposedActionId}`}
          value={noteText}
