...
import { batchDismissCapturesAction } from './batchReviewActions'

interface Props {
  captures: GeneralCaptureItem[]
  players: PlayerOption[]
}

export function CapturesBatchPanel({ captures, players }: Props) {
  ...
  const visible = captures.filter(c => !dismissed.has(c.id))
  ...
  function handleConfirm() {
    ...
    startTransition(async () => {
      const res = await batchDismissCapturesAction(ids)
      if (res.ok) {
        setDismissed(prev => {
          const next = new Set(prev)
        ...
        setSelected(new Set())
        setConfirming(false)
        setResult(`${res.dismissed} capture${res.dismissed !== 1 ? 's' : ''} dismissed.`)
      } else {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-status-red/5 border border-status-red/20">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-text-primary">
              Dismiss {selected.size} {selected.size === 1 ? 'capture' : 'captures'}?
            </p>
            <p className="text-xs text-text-muted mt-0.5">
              These will be removed from the review queue. This cannot be undone.
            </p>
      ...
      {/* Individual capture cards with checkboxes */}
      <div className="space-y-3">
        {visible.map(capture => (
          <div key={capture.id} className="flex items-start gap-3">
            <button
              onClick={() => toggle(capture.id)}
              className="mt-3 shrink-0 text-text-muted hover:text-lime transition-colors"
              aria-label={selected.has(capture.id) ? 'Deselect' : 'Select'}
            >
              {selected.has(capture.id)
                ? <CheckSquare className="w-4 h-4 text-lime" />
                : <Square className="w-4 h-4" />
              }
            </button>
            <div className="flex-1 min-w-0">
              <GeneralCaptureDraftCard capture={capture} players={players} />
            </div>
          </div>
        ))}
      </div>
