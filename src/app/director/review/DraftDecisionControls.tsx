...
export function DraftDecisionControls({ proposedActionId }: Props) {
  ...
  function handleDecision(decision: DraftDecision) {
    startTransition(async () => {
      const res = await updateStructuredDraftDecisionAction(
        proposedActionId,
        decision,
  ...
  return (
    <div className="space-y-3 pt-3 border-t border-border">
      ...
      <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border text-[11px] text-text-muted">
        ...
        <span>
          Approving does not apply changes yet. It only marks this draft as ready for a future
          application step.
        </span>
      </div>
