...
export default async function AcademySetupPage() {
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-text-secondary text-sm">Please sign in to access setup.</p>
      </div>
    )
  }
  ...
  const group1 = STEP_DEFS.slice(0, 3)
  const group2 = STEP_DEFS.slice(3, 6)
  const group3 = STEP_DEFS.slice(6, 7)
  const advanced = STEP_DEFS.slice(7)
  ...
  return (
    <div className="animate-fade-in p-6 space-y-5 max-w-2xl">
      ...
      <div>
        ...
        <h1 className="page-title">Academy Setup Checklist</h1>
      ...
      {/* ── Group 2: Build Your Operating System ── */}
      <section className="space-y-2">
        <div className="flex items-center justify-between px-1">
      ...
      <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl bg-lime/5 border border-lime/20 text-xs text-text-secondary">
        ...
        <span>
          This is the step-by-step setup checklist. For the guided DONNA setup, visit{' '}
          <Link href="/director/onboarding" className="text-lime hover:underline">Director Onboarding</Link>.
        </span>
      </div>
