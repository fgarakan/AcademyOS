...
export function BlockContentPickerCard({
  ...
  return (
    <div className="space-y-2">
      {/* Section label */}
      <p className="text-[10px] uppercase tracking-widest text-text-muted pl-7">
        Curriculum Content
        {initialAssigned.length > 0 && (
          <span className="ml-1.5 font-mono text-lime normal-case tracking-normal">
            {initialAssigned.length}
          </span>
        )}
      </p>
            ...
            .map(item => (
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 opacity-30 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-status-red/10 text-text-muted hover:text-status-red disabled:opacity-20"
                >
                  <X className="w-3 h-3" />
                </button>
              </li>
      ...
      <div className="pl-7">
        ...
        >
          ...
          {isPending ? 'Updating…' : 'Add Content'}
        </button>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-xs text-text-muted -mt-1">
            For <span className="text-text-secondary font-medium">{blockName}</span> — choose a drill, game, cue, or focus item.
          </p>

          {/* In-flight indicator */}
          {isPending && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-surface-raised border border-border">
              <span className="w-3 h-3 border-2 border-lime border-t-transparent rounded-full animate-spin shrink-0" />
              <p className="text-[10px] text-text-muted">Saving…</p>
            </div>
          )}
          ...
          <p className="text-[10px] text-text-muted pt-2 border-t border-border">
            Internal planning only. Parent and player visibility is controlled separately.
          </p>
        </div>
      </Modal>
    </div>
