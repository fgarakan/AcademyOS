...
function safeBlockType(t: string): DbBlockType {
  return VALID_BLOCK_TYPES.has(t) ? (t as DbBlockType) : 'free'
}
...
export interface CreateClassTemplateWithBlocksInput {
  name: string
  description: string
  templateType: string
  ballLevel: string
  ...
  blocks: BlockDraftInput[]
}

export interface CreateClassTemplateWithBlocksResult {
  ok: boolean
  error: string | null
  templateId: string | null
}

export async function createClassTemplateWithBlocksAction(
  input: CreateClassTemplateWithBlocksInput,
): Promise<CreateClassTemplateWithBlocksResult> {
  ...
  const trackParts = [
    typeof input.templateType === 'string' ? input.templateType.trim() : '',
    typeof input.ballLevel    === 'string' ? input.ballLevel.trim()    : '',
    typeof input.groupType    === 'string' ? input.groupType.trim()    : '',
  ].filter(Boolean)
  const track = trackParts.length > 0 ? trackParts.join(' · ') : null

  const tags: string[] = ['source:builder_v1', 'status:draft']
  if (input.ballLevel) tags.push(`level:${String(input.ballLevel).trim()}`)
  ...
  if (Array.isArray(input.blocks) && input.blocks.length > 0) {
    const blockRows = input.blocks.map((block, idx) => ({
      ...
      type:         safeBlockType(typeof block.type === 'string' ? block.type : 'free'),
