...
import {
  ...
  Loader2, ClipboardList, RotateCcw, Info, Sparkles,
} from 'lucide-react'
...
export function AssessmentStudioForm({
  ...
  useEffect(() => {
    return onGoalSessionCompleted(detail => {
      if (detail.workflowId !== 'assessment_completion') return
      const plan = buildWorkflowExecutionPlan(detail)
      if (plan) setDonnaPlan(plan)
    })
  }, [])

  async function handleDonnaAssessmentConfirm() {
    if (!donnaPlan) return
    ...
    const result = await submitDonnaActionDraft({
      rawInput: `Assessment draft: ${ans['player_name'] ?? playerFirstName ?? 'Player'} — ${ans['assessment_domain'] ?? 'domain'} domain`,
      actionLabel: `Assessment Draft — ${playerFirstName ?? ans['player_name'] ?? 'Player'} — ${ans['assessment_domain'] ?? 'General'}`,
      targetModule: 'assessment_draft_v1',
      proposedPayload: {
        playerId,
        player_name:        ans['player_name'],
        assessment_domain:  ans['assessment_domain'],
        observation:        ans['observation'],
        performance_rating: ans['performance_rating'],
        recommendation:     ans['recommendation'],
        parent_visibility:  ans['parent_visibility'],
      },
      riskLevel: 'low',
    })
    ...
    const verification = buildWorkflowVerificationResult(submitResult, ans['player_name'] ?? playerFirstName ?? 'Assessment')

    if (verification.verified) {
      const summary = buildWorkflowCompletionSummary('assessment_completion', verification, ans)
      setDonnaCompletion(summary)
      setDonnaPlan(null)
    } else {
      setDonnaError(verification.failureReason ?? result.error ?? 'Failed to save assessment draft.')
    }
      ...
      {donnaCompletion && (
        <div className="rounded-xl border border-status-green/30 bg-status-green/5 p-4 space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-status-green">Assessment draft queued for review</p>
          </div>
          <p className="text-xs text-text-secondary leading-relaxed">{donnaCompletion.donnaMessage}</p>
        </div>
      )}
      {donnaPlan && !donnaCompletion && (
        <div className="rounded-xl border border-lime/25 bg-lime/5 overflow-hidden">
          <div className="px-4 py-3 border-b border-lime/15 flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-lime">DONNA collected these assessment answers</p>
          </div>
          ...
          <div className="px-4 py-3 border-t border-lime/15 flex items-center gap-2">
            <button
              type="button"
              onClick={handleDonnaAssessmentConfirm}
            ...
            >
              ...
              {donnaSubmitting ? 'Saving…' : 'Confirm & Queue Assessment Draft'}
            </button>
            <button type="button" onClick={() => { setDonnaPlan(null); setDonnaError(null) }} disabled={donnaSubmitting} className="btn-ghost text-xs px-3 py-2">
              Dismiss
            </button>
          </div>
        </div>
