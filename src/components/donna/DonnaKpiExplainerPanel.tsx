...
function fireDonna(prompt: string) {
  window.dispatchEvent(new CustomEvent('donna:open', { detail: { prompt } }))
}

// ── Shared chip ───────────────────────────────────────────────────────────────

function DonnaChip({ label, prompt }: { label: string; prompt: string }) {
  return (
    <button
      onClick={() => fireDonna(prompt)}
  ...
  return (
    <div className="rounded-xl border border-lime/20 bg-lime/5 px-4 py-3 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {chips.map(chip => (
          <DonnaChip key={chip.label} label={chip.label} prompt={chip.prompt} />
        ))}
      </div>
...
export function DonnaDashboardPresenceCTA(props: DonnaDashboardPresenceCTAProps) {
  ...
  return (
    <div className="rounded-xl border border-lime/20 bg-lime/5 px-4 py-3 space-y-2">
      ...
      <DonnaChip label="What should I do first?" prompt={prompt} />
      <p className="text-[10px] text-text-muted">
...
export function DonnaPlayersPresenceCTA(props: DonnaPlayersPresenceCTAProps) {
  ...
  return (
    <div className="rounded-xl border border-lime/20 bg-lime/5 px-4 py-3 space-y-2">
      ...
      <DonnaChip label="Who needs attention?" prompt={prompt} />
      <p className="text-[10px] text-text-muted">
