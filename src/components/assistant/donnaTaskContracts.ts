...
export const DONNA_TASK_CONTRACTS: Record<DonnaTaskId, DonnaTaskContract> = {
  ...
  create_class_template: {
    ...
    description:
      'Guide the director through creating a reusable, curriculum-aligned class template with named blocks and time allocations.',
    requiredFields: [
      { fieldId: 'level',    label: 'Curriculum Level', required: true,  example: 'Orange 2' },
      { fieldId: 'duration', label: 'Class Duration',   required: true,  example: '60 minutes' },
      { fieldId: 'blocks',   label: 'Session Blocks',   required: true,  example: 'Warm-Up, Rally Skills, Point Play, Matches' },
    ],
    optionalFields: [
      { fieldId: 'template_name',      label: 'Template Name',      required: false, example: 'Orange 2 — Skills + Match' },
      { fieldId: 'coach_instructions', label: 'Coach Instructions', required: false },
      { fieldId: 'curriculum_focus',   label: 'Curriculum Focus',   required: false },
      { fieldId: 'class_type',         label: 'Class Type',         required: false, example: 'Group lesson' },
    ],
    questionSequence: [
      { order: 1, fieldId: 'level',    question: 'What level is this class template for? (e.g. Orange 2, Red 1, Yellow 3)' },
      { order: 2, fieldId: 'duration', question: 'How long is the class? (e.g. 60 minutes, 90 minutes)' },
      { order: 3, fieldId: 'blocks',   question: 'What blocks should be included? (e.g. warm-up, rally skills, point play, matches)' },
      { order: 4, fieldId: 'template_name', question: 'What should we call this template? (I can suggest one based on the level and blocks)' },
    ],
