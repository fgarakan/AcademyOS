...
export function AcademyDnaReviewStep({ draft, updateDraft: _updateDraft, onNext, onPrev, onEditStep }: Props) {
  const hasAnyContent = !!(
  ...
  return (
    <div>
      ...
      {/* 2-column layout: DNA card right (mobile: card first), detail left */}
      <div className="lg:grid lg:grid-cols-[1fr_272px] lg:gap-6 lg:items-start">

        {/* Right column — DNA identity card (shows above detail on mobile) */}
        <div className="order-1 lg:order-2 lg:sticky lg:top-6 mb-6 lg:mb-0">
          <AcademyDnaSummaryCard draft={draft} onEditStep={onEditStep} />
        </div>

        {/* Left column — full detail sections + DONNA summary + nav */}
