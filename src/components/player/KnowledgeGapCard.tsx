// Knowledge Gap Card — Sprint 233
// Director/coach facing. Displays detected knowledge gaps from IdpKnowledgeGap[].
// Read-only. No mutations. Never shown to player or parent.

import type { ReactNode } from 'react'
import { BookOpen, AlertTriangle, AlertCircle, Info, HelpCircle } from 'lucide-react'
...
interface Props {
  gaps: IdpKnowledgeGap[]
}

export function KnowledgeGapCard({ gaps }: Props) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-surface-raised border border-border flex items-center justify-center shrink-0">
              <BookOpen className={`w-4 h-4 ${highCount > 0 ? 'text-status-red' : 'text-text-muted'}`} />
            </div>
            <div>
              <p className="font-semibold text-text-primary text-sm">Knowledge Gaps</p>
              <p className="text-text-muted text-xs">Based on curriculum and gate data</p>
            </div>
          </div>
      ...
      <CardContent>
        {gaps.length === 0 ? (
          <EmptyState
            ...
            description="Curriculum coverage and gate distribution look balanced."
            className="py-6"
          />
        ) : (
                ...
                <li key={i} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {gap.suggested_module_domain && (
                        <span className="text-[9px] uppercase tracking-widest text-lime border border-lime/30 px-1 rounded">
                          {gap.suggested_module_domain}
                        </span>
                      )}
                    </div>
